...
export const confirmTicketCreation = async (interaction) => {
    ...
    try {
        // Disable buttons immediately to prevent double click visually and satisfy interaction
        await interaction.update({
            components: [
                new ActionRowBuilder().addComponents(
                    new ButtonBuilder().setCustomId('ticket_confirm_yes').setLabel('Yes ✅').setStyle(ButtonStyle.Success).setDisabled(true),
                    new ButtonBuilder().setCustomId('ticket_confirm_no').setLabel('No').setStyle(ButtonStyle.Secondary).setDisabled(true)
                )
            ]
        });
    } catch (e) {
        console.error('Failed to update interaction with disabled buttons on create:', e);
    }
    ...
    try {
        ...
        // Use editReply because we already updated the interaction
        ...
        console.error('Error during ticket creation confirm:', err);
    } finally {
...
export const executeCloseTicket = async (interaction) => {
    const key = `close_${interaction.channelId}`;
    ...
    try {
        await interaction.update({
            components: [
                new ActionRowBuilder().addComponents(
                    new ButtonBuilder().setCustomId('ticket_close_confirm_yes').setLabel('نعم').setStyle(ButtonStyle.Danger).setDisabled(true),
                    new ButtonBuilder().setCustomId('ticket_close_confirm_no').setLabel('لا').setStyle(ButtonStyle.Secondary).setDisabled(true)
                )
            ]
        });
    } catch (e) {
        console.error('Failed to disable buttons on close confirm:', e);
    }
    ...
    try {
        ...
        // Use editReply instead of update because we updated the interaction
        try {
        ...
        console.error('Error during ticket closing:', err);
    } finally {
        activeOperations.delete(key);
    }
};
...
export const handleClaimTicket = async (interaction) => {
    ...
    if (!targetTicket) {
        try {
            await interaction.reply({ content: 'عذراً، لم يتم العثور على التيكيت فى قاعدة البيانات.', ephemeral: true });
        } catch {
            await interaction.followUp({ content: 'عذراً، لم يتم العثور على التيكيت فى قاعدة البيانات.', ephemeral: true });
        }
        return;
    }

    const key = `claim_${targetTicket.channelId}`;
    if (activeOperations.has(key)) return;
    activeOperations.add(key);

    try {
        // Disable claim button immediately to prevent multiple clicks and satisfy interaction
        if (interaction.channelId === config.claimChannelId) {
            await interaction.update({
                components: [
                    new ActionRowBuilder().addComponents(
                        new ButtonBuilder().setCustomId('ticket_claim').setLabel('استلام التيكيت').setStyle(ButtonStyle.Primary).setEmoji('✅').setDisabled(true)
                    )
                ]
            });
        } else {
        ...
        if (targetTicket.claimedBy) {
            const claimedEmbed = new EmbedBuilder()
                ...
                .setColor('#8B2FF3');
            await interaction.followUp({ embeds: [claimedEmbed], ephemeral: true });
            return;
        }

        const newTicket = await claimTicket(targetTicket.channelId, interaction.user.id);
        if (!newTicket) {
            await interaction.followUp({ content: 'فشل استلام التيكيت، ربما استلمه شخص آخر بالفعل.', ephemeral: true });
            return;
        }
        ...
        console.error('Error during ticket claiming:', err);
    } finally {
        activeOperations.delete(key);
    }
};
...
export const handleReopenTicket = async (interaction) => {
    const key = `reopen_${interaction.channelId}`;
    if (activeOperations.has(key)) return;
    activeOperations.add(key);
    ...
    try {
        await interaction.update({
            components: [
                new ActionRowBuilder().addComponents(
                    new ButtonBuilder().setCustomId('ticket_open').setLabel('Open').setStyle(ButtonStyle.Success).setDisabled(true),
                    new ButtonBuilder().setCustomId('ticket_delete').setLabel('Delete').setStyle(ButtonStyle.Danger).setDisabled(true)
                )
            ]
        });
    } catch (e) {
        console.error('Failed to disable buttons on reopen:', e);
    }
    ...
    try {
        ...
        await interaction.editReply({ content: `**Ticket reopened by <@${interaction.user.id}>**`, embeds: [], components: [] });
        ...
        console.error('Error during ticket reopening:', err);
    } finally {
        activeOperations.delete(key);
    }
};
...
export const handleDeleteTicket = async (interaction) => {
    const key = `delete_${interaction.channelId}`;
    if (activeOperations.has(key)) return;
    activeOperations.add(key);
    ...
    try {
        ...
        console.error('Failed to disable buttons on delete:', e);
    }
    ...
    try {
        ...
        console.error('Error during ticket deleting:', err);
    } finally {
        activeOperations.delete(key);
    }
};
